...
//
static bool whisper_model_load(struct whisper_model_loader * loader, whisper_context & wctx, const int verbose) {
    if (verbose) Rprintf("%s: loading model\n", __func__);
    ...
    // verify magic
    {
        ...
        if (magic != 0x67676d6c) {
          if (verbose) Rprintf("%s: invalid model data (bad magic)\n", __func__);
            return false;
        }
    }
    ...
    //load hparams
    {
        ...
        if (wctx.wtype == GGML_TYPE_COUNT) {
          if (verbose) Rprintf("%s: invalid model (bad ftype value %d)\n", __func__, model.hparams.ftype);
            return false;
        }

        const size_t scale = model.hparams.ftype ? 1 : 2;

        if (verbose) Rprintf("%s: n_vocab       = %d\n", __func__, hparams.n_vocab);
        if (verbose) Rprintf("%s: n_audio_ctx   = %d\n", __func__, hparams.n_audio_ctx);
        if (verbose) Rprintf("%s: n_audio_state = %d\n", __func__, hparams.n_audio_state);
        if (verbose) Rprintf("%s: n_audio_head  = %d\n", __func__, hparams.n_audio_head);
        if (verbose) Rprintf("%s: n_audio_layer = %d\n", __func__, hparams.n_audio_layer);
        if (verbose) Rprintf("%s: n_text_ctx    = %d\n", __func__, hparams.n_text_ctx);
        if (verbose) Rprintf("%s: n_text_state  = %d\n", __func__, hparams.n_text_state);
        if (verbose) Rprintf("%s: n_text_head   = %d\n", __func__, hparams.n_text_head);
        if (verbose) Rprintf("%s: n_text_layer  = %d\n", __func__, hparams.n_text_layer);
        if (verbose) Rprintf("%s: n_mels        = %d\n", __func__, hparams.n_mels);
        if (verbose) Rprintf("%s: ftype         = %d\n", __func__, model.hparams.ftype);
        if (verbose) Rprintf("%s: qntvr         = %d\n", __func__, qntvr);
        if (verbose) Rprintf("%s: type          = %d\n", __func__, model.type);
        ...
        // print memory requirements
        {
            ...
            if (verbose) Rprintf("%s: mem required  = %7.2f MB (+ %7.2f MB per decoder)\n", __func__,
    ...
    // load vocab
    {
        ...
        if (n_vocab < model.hparams.n_vocab) {
          if (verbose) Rprintf("%s: adding %d extra tokens\n", __func__, model.hparams.n_vocab - n_vocab);
...

    {
        ...
        if (verbose) Rprintf("%s: model ctx     = %7.2f MB\n", __func__, ctx_size/(1024.0*1024.0));
    }
    ...
    // create the ggml context
    {
        ...
        if (!model.ctx) {
          if (verbose) Rprintf("%s: ggml_init() failed\n", __func__);
            return false;
        }
    }
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            if (model.tensors.find(name) == model.tensors.end()) {
              if (verbose) Rprintf("%s: unknown tensor '%s' in model file\n", __func__, name.data());
                return false;
            }

            auto tensor = model.tensors[name.data()];
            if (ggml_nelements(tensor) != nelements) {
              if (verbose) Rprintf("%s: tensor '%s' has wrong size in model file\n", __func__, name.data());
              if (verbose) Rprintf("%s: shape: [%d, %d, %d], expected: [%d, %d, %d]\n",
                        __func__, ne[0], ne[1], ne[2], (int) tensor->ne[0], (int) tensor->ne[1], (int) tensor->ne[2]);
                return false;
            }

            if (tensor->ne[0] != ne[0] || tensor->ne[1] != ne[1] || tensor->ne[2] != ne[2]) {
              if (verbose) Rprintf("%s: tensor '%s' has wrong shape in model file: got [%d, %d, %d], expected [%d, %d, %d]\n",
            ...
            if ((nelements*bpe)/ggml_blck_size(tensor->type) != ggml_nbytes(tensor)) {
              if (verbose) Rprintf("%s: tensor '%s' has wrong size in model file: got %zu, expected %zu\n",
        ...
        if (verbose) Rprintf("%s: model size    = %7.2f MB\n", __func__, total_size/1024.0/1024.0);

        if (model.n_loaded == 0) {
          if (verbose) Rprintf("%s: WARN no tensors loaded from model file - assuming empty model for testing\n", __func__);
        } else if (model.n_loaded != (int) model.tensors.size()) {
          if (verbose) Rprintf("%s: ERROR not all tensors loaded from model file - expected %zu, got %d\n", __func__, model.tensors.size(), model.n_loaded);
            return false;
        }
    }
...
struct whisper_state * whisper_init_state(whisper_context * ctx, const int verbose) {
    ...
    if (!kv_cache_init(ctx->model.hparams, scale * MEM_REQ_KV_SELF.at(ctx->model.type), state->decoders[0].kv_self, ctx->itype, ctx->model.hparams.n_text_ctx)) {
        if (verbose) Rprintf("%s: kv_cache_init() failed for self-attention cache\n", __func__);
        delete state;
        return nullptr;
    }

    {
      ...
      if (verbose) Rprintf("%s: kv self size  = %7.2f MB\n", __func__, memory_size / 1024.0 / 1024.0);
    }
    ...
    if (!kv_cache_init(ctx->model.hparams, scale * MEM_REQ_KV_CROSS.at(ctx->model.type), state->kv_cross, ctx->itype, ctx->model.hparams.n_audio_ctx)) {
      if (verbose) Rprintf("%s: kv_cache_init() failed for cross-attention cache\n", __func__);
        delete state;
        return nullptr;
    }

    {
      ...
      if (verbose) Rprintf("%s: kv cross size = %7.2f MB\n", __func__, memory_size / 1024.0 / 1024.0);
    }

#ifdef WHISPER_USE_COREML
    const auto path_coreml = whisper_get_coreml_path_encoder(ctx->path_model);

    if (verbose) Rprintf("%s: loading Core ML model from '%s'\n", __func__, path_coreml.c_str());
    if (verbose) Rprintf("%s: first run on a device may take a while ...\n", __func__);

    state->ctx_coreml = whisper_coreml_init(path_coreml.c_str());
    if (!state->ctx_coreml) {
      if (verbose) Rprintf("%s: failed to load Core ML model from '%s'\n", __func__, path_coreml.c_str());
#ifndef WHISPER_COREML_ALLOW_FALLBACK
        return nullptr;
#endif
    } else {
      if (verbose) Rprintf("%s: Core ML model loaded\n", __func__);
    }
#endif
...
struct whisper_context * whisper_init_from_file_no_state(const char * path_model, const int verbose) {

    if (verbose) Rprintf("%s: loading model from '%s'\n", __func__, path_model);

    auto fin = std::ifstream(path_model, std::ios::binary);
    if (!fin) {
      if (verbose) Rprintf("%s: failed to open '%s'\n", __func__, path_model);
        return nullptr;
    }
    ...
    auto ctx = whisper_init_no_state(&loader, verbose);
...
struct whisper_context * whisper_init_from_buffer_no_state(void * buffer, size_t buffer_size) {
    ...
    return whisper_init_no_state(&loader, 1);
}

struct whisper_context * whisper_init_no_state(struct whisper_model_loader * loader, const int verbose) {
    ggml_time_init();

    whisper_context * ctx = new whisper_context;

    if (!whisper_model_load(loader, *ctx, verbose)) {
...
struct whisper_context * whisper_init_from_file(const char * path_model, const int verbose) {
    whisper_context * ctx = whisper_init_from_file_no_state(path_model, verbose);
    if (!ctx) {
        return nullptr;
    }

    ctx->state = whisper_init_state(ctx, verbose);
    if (!ctx->state) {
        whisper_free(ctx);
        return nullptr;
    }
...
struct whisper_context * whisper_init_from_buffer(void * buffer, size_t buffer_size) {
    ...
    ctx->state = whisper_init_state(ctx, 1);
    if (!ctx->state) {
        whisper_free(ctx);
        return nullptr;
    }
...
struct whisper_context * whisper_init(struct whisper_model_loader * loader) {
    whisper_context * ctx = whisper_init_no_state(loader, 1);
    if (!ctx) {
        return nullptr;
    }
...
int whisper_decode(struct whisper_context * ctx, const whisper_token * tokens, int n_tokens, int n_past, int n_threads) {
    ...
    if (ctx->state == nullptr) {
        Rprintf("%s: ERROR state was not loaded.\n", __func__);
        return false;
    }
...
int whisper_full_parallel(
    ...
    for (int i = 0; i < n_processors - 1; ++i) {
        ...
        states.push_back(whisper_init_state(ctx, 1));
